...
직무 적합(가장 중요): 지원자는 시스템 엔지니어링·기술 프로그램/프로젝트 관리(TPM/PM)·양산/제품화·요구사항/V&V·리스크/스코프 관리 전문가다. 반도체/SW/HW의 '설계·개발·공정·회로설계·펌웨어·검증' 같은 순수 엔지니어링 직무는 도메인(반도체/로봇 등)이 맞아도 지원자 직무가 아니므로 최대 2점. SE/TPM/PM/양산·프로그램관리·PMO·제품화 직무일 때만 3점 이상 가능. (도메인은 반도체/로봇/자율주행/항공우주에 '몸담는 회사'라는 뜻이지, 그 분야 설계자가 되라는 뜻이 아님)
회사 규모(중요): 대기업 또는 유망 기술 스타트업(특히 NPU/반도체/로보틱스/eVTOL/자율주행 분야)만 3점 이상 가능. 일반 중소·중견 제조/부품/장비/SI 기업은 최대 2점(스타트업은 이 제한 없음).

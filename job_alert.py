...
# 크롤 파라미터
WANTED_KEYWORDS = ["Technical Program Manager", "Technical Product Manager", "Product Manager",
                   "프로덕트 매니저", "프로그램 매니저", "시스템 엔지니어", "양산 PM",
                   "로보틱스 PM", "자율주행 PM", "하드웨어 PM", "반도체 PM", "제품 기획"]
WANTED_LIMIT_PER_KW = 25   # 키워드당 최신 공고 수
WANTED_MAX_DETAIL   = 60   # 상세조회(설명 확보) 최대 건수 — IP차단/속도 보호
LINKEDIN_TERMS = ["Technical Program Manager semiconductor", "Systems Engineer NPU",
                  ...
                  "Technical Product Manager robotics", "Product Manager autonomous driving",
                  "Technical Product Manager hardware", "product manager semiconductor"]
LINKEDIN_MAX_DETAIL = 25    # 상세조회 최대 건수 — IP차단/속도 보호

# 국내 잡보드(사람인/잡코리아/인크루트) 검색 키워드 — 도메인·직무 타겟팅
# 국내보드 검색 — 지원자 '직무'(SE/TPM/PM·양산·제품화) 중심. 설계/개발/공정 아님.
DOMESTIC_KEYWORDS = ["기술 프로그램 매니저", "테크니컬 프로그램 매니저", "프로덕트 매니저",
...
# 현대자동차그룹사 — 현직이라 제외(회사명/제목에 포함되면 차단). 필요시 추가.
COMPANY_BLOCK = ["현대자동차", "현대차", "현대모비스", "현대글로비스", "현대오토에버",
                 "현대트랜시스", "현대위아", "현대케피코", "현대제철", "현대로템",
                 "현대건설", "현대엔지니어링", "기아", "hyundai motor", "hyundai mobis",
                 "hyundai glovis", "glovis", "autoever", "hmg"]

def _company_ok(job):
    blob = (job.get("company", "") + " " + job.get("title", "")).lower()
    return not any(b in blob for b in COMPANY_BLOCK)

def passes_filter(job):
    if not _company_ok(job):          # 현대차그룹사 제외
        return False
...
#  크롤러 — 원티드 (JSON API)
# ══════════════════════════════════════════════════
def _wanted_search(keyword, limit, offset=0):
    url = ("https://www.wanted.co.kr/api/v4/jobs"
           f"?country=kr&job_sort=job.latest_order&locations=all&years=-1"
           f"&limit={limit}&offset={offset}&keyword={requests.utils.quote(keyword)}")
...
def crawl_wanted():
    ...
    for kw in WANTED_KEYWORDS:
        for off in (0, WANTED_LIMIT_PER_KW):   # 최신공고 2페이지 훑기(keyword가 느슨해서)
            try:
                for j in _wanted_search(kw, WANTED_LIMIT_PER_KW, off):
                    jid = j.get("id")
                    if jid and jid not in listings:
                        listings[jid] = j
                time.sleep(0.4)   # 예의상 지연(IP 차단 방지)
            except Exception as e:
                print(f"  [원티드/{kw}@{off}] 검색 오류: {e}")
...
- 1~2: 부분매칭·커리어 하향, 또는 중소·중견/설계·개발 직무
직무 적합(가장 중요): 지원자는 시스템 엔지니어링·기술 프로그램/프로젝트 관리(TPM/PM)·양산/제품화·요구사항/V&V·리스크/스코프 관리 전문가다. 반도체/SW/HW의 '설계·개발·공정·회로설계·펌웨어·검증' 같은 순수 엔지니어링 직무는 도메인이 맞아도 지원자 직무가 아니므로 최대 2점. SE/TPM/PM/양산·프로그램관리·PMO·제품화 직무일 때만 3점 이상 가능.
회사 규모/성격(중요): (1) 대기업, 또는 (2) 유망 기술 스타트업(유니콘·시리즈B+ 등 잘 성장하는 테크 스타트업: 예 토스·당근·몰로코·리벨리온·니어스랩·Bear Robotics 등)이면 3점 이상 가능. 특히 유망 스타트업의 SE/TPM/PM 직무는 관심 도메인(반도체/로봇/자율주행/항공우주)이 정확히 아니어도 3점 가능(도메인까지 정확히 맞는 대기업/스타트업은 4~5점). 단 일반 중소·중견 제조/부품/장비/SI 기업은 최대 2점.
회사명 처리(중요): 회사명이 비어 있거나 처음 보는 회사여도 '중소'로 단정하지 마라. 그 경우 회사규모는 중립으로 두고 직무(SE/TPM/PM 여부)와 도메인만으로 판단하라. 회사명 없음 자체를 감점 사유로 쓰지 마라.
현대자동차그룹사(현대차/기아/현대모비스/현대글로비스/현대오토에버 등)는 현직이므로 1점.
방산·항공우주 프라임(Lockheed Martin/Collins/Raytheon/RTX/Anduril/Northrop 등)의 한국 포지션은 지원자의 대한항공 UH-60·글로벌 파트너 협상 이력과 직접 부합 → 도메인 적합으로 보고 대기업 규모라 3점 이상 가능.
...
def run(dry=False):
    ...
    broad = crawl_linkedin()                            # 링크드인(글로벌) → 엄격 필터
    domestic = crawl_wanted() + crawl_saramin() + crawl_jobkorea() + crawl_incruit()  # 국내 타겟 → 완화 필터
    ...
    print(f"   총 수집 {len(broad)+len(domestic)+len(defense)+len(samsung)+len(samsung_ds)}건 "
          f"(링크드인 {len(broad)} / 국내 {len(domestic)} / 방산 {len(defense)} / 삼성 {len(samsung)} / 삼성DS {len(samsung_ds)})", flush=True)
...
def _selftest():
    ...
    # 2c-2) 현대차그룹사 차단
    assert not _company_ok({"company": "현대모비스", "title": "시스템 엔지니어"}), "현대모비스 미차단"
    assert not _company_ok({"company": "Hyundai Motor Company", "title": "Robotics PM"}), "현대차 미차단"
    assert not passes_filter({"title": "Systems Engineer", "company": "현대오토에버",
                              "location": "경기", "desc": "반도체 시스템"}), "현대오토에버 필터 통과"
    assert _company_ok({"company": "삼성전자", "title": "반도체 PM"}), "삼성전자 오차단"
    assert _company_ok({"company": "현대중공업", "title": "엔지니어"}), "현대중공업(비 HMG) 오차단"
    print("[OK] HMG 그룹사 차단 (삼성·현대중공업 등 비대상은 통과)")
